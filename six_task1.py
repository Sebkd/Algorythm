...
Размер "list_b =" 184
Размер "tuple_c =" 120
Списки имеют разный размер, но занимают один и тот же размер памяти
размер листа меньше чем должен быть по формуле 40+8*длина
получается Python делает ссылки на одни и те же числа, при увеличении 
массива растет выделяемый объем памяти
кортеж наоборот имеет больший объем памяти, чем по формуле
24+8*длина, с связано не знаю. При увеличении размера кортежа
выделение памяти растет
'''